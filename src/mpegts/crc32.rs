...
impl<T: Buf> Buf for WithCrc32<T> {
    ...
    fn advance(&mut self, cnt: usize) {
        self.crc32.update(&self.stream.chunk()[..cnt]);
        self.stream.advance(cnt);
    }
}

unsafe impl<T: BufMut> BufMut for WithCrc32<T> {
    fn remaining_mut(&self) -> usize {
        self.stream.remaining_mut()
    }

    unsafe fn advance_mut(&mut self, cnt: usize) {
        let slice = unsafe {
            &mut *(self.stream.chunk_mut() as *mut bytes::buf::UninitSlice
                as *mut [MaybeUninit<u8>] as *mut [u8])
        };

        // MaybeUninit::assume_init_ref(self.stream.chunk_mut()[..cnt].as_uninit_slice_mut())

        self.crc32.update(&slice[..cnt]);
        unsafe { self.stream.advance_mut(cnt) };
    }

    fn chunk_mut(&mut self) -> &mut bytes::buf::UninitSlice {
        self.stream.chunk_mut()
    }
}
