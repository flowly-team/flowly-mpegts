...
use bytes::Bytes;
use flowly::{
    Chunk, ChunkRef, DataFrame, EncodedFrame, Fourcc, Frame, FrameFlags, FrameSource, MemBlock,
};
...
impl<S: FrameSource> FrameSource for Mpeg2TsSource<S> {
    ...
    fn kind(&self) -> flowly::FrameSourceKind {
        self.inner.kind()
    }
...
impl<S: FrameSource> DataFrame for Mpeg2TsFrame<S> {
    ...
    fn chunks(&self) -> impl Send + Iterator<Item = flowly::ChunkRef<Self::Source, Self::Chunk>> {
        std::iter::once(ChunkRef::new(&self.source, self.payload.as_ref()))
    }

    fn into_chunks(self) -> impl Send + Iterator<Item = flowly::Chunk<Self::Source, Self::Chunk>> {
        std::iter::once(Chunk::new(self.source, self.payload))
    }
